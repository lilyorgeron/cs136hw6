...
class VCG:
    ...
    def compute(slot_clicks, reserve, bids):
        ...
        base_case = max(reserve, valid_bids[num_slots][1]) if len(valid_bids) > num_slots else reserve
        ...
        def total_payment(k):
            ...
            if k == len(allocation) - 1:
                payments[k] = c[k] * base_case
 
                return payments[k]
            payments[k] = (c[k] - c[k+1])*allocated_bids[k+1][1] + payments[k+1]
            return payments[k]
        ...
        per_click_payments = norm(
            [total_payment(k) for k in range(len(allocation) - 1, -1, -1)][::-1])
